...
def main(argv: list[str] = []) -> int:
    """
    Main entry point for the `{{ cookiecutter.python_script_name }}` command line utility.
    Args:
        argv (list[str]], optional): Arguments to use when executing `{{ cookiecutter.python_script_name }}`. Defaults to `sys.argv`.
    Returns:
    ...
    parser = argparse.ArgumentParser(prog="{{ cookiecutter.python_script_name }}", add_help=False)
    parser.add_argument(
        "--debug",
        action="store_true",
        default=False,
